...
def get_local_tz(local_tz_override: str | None = None) -> ZoneInfo:
    ...
    # Use tzlocal to get the IANA timezone name
    try:
    ...
    # Fallback: try to get from datetime.now().astimezone()
    tzinfo = datetime.now().astimezone(tz=None).tzinfo
    if tzinfo is not None:
        # Try to use the key attribute if available (Python 3.9+)
        tz_name = getattr(tzinfo, 'key', None)
        if tz_name:
            return ZoneInfo(tz_name)
        # Otherwise, try str(tzinfo) but warn user
        try:
            return ZoneInfo(str(tzinfo))
        except Exception:
            pass
    raise McpError("Could not determine local timezone as a valid IANA name. Please specify --local-timezone explicitly.")
...
async def serve(local_timezone: str | None = None) -> None:
    ...
    # print("\n[MCP] Tariff Lookup MCP server starting...")
    # print(f"[MCP] To connect from VS Code, use the following command:")
    # print(f"[MCP]   python server.py [--local-timezone <IANA_TZ_NAME>]")
    # print("[MCP] Transport: stdio (standard input/output)")
    # print("[MCP] Example: python server.py --local-timezone Europe/Berlin\n")
